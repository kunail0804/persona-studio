...
An `image` row can be pointed to from four places: `scenario.portrait_id`,
`character.portrait_id`, `message.image_id`, and `scenario.background` when it
holds an image id rather than the `'portrait'` sentinel or NULL. Deleting a
scenario cascades away the rows that reference it (`character`, `instance`,
`message`, ...), but SQLite's `ON DELETE CASCADE` only removes rows — it never
touches the `image` row itself or the PNG file on disk, since nothing in the
schema points the other way. This module is the part that does.
"""
    ...
    orphan_ids = [
        ...
        for row in con.execute(
            ...
            WHERE id NOT IN (SELECT portrait_id FROM scenario WHERE portrait_id IS NOT NULL)
              ...
              AND id NOT IN (
                  SELECT background FROM scenario
                  WHERE background IS NOT NULL AND background != 'portrait'
              )
            """
        ).fetchall()
    ]
